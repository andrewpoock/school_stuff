...
class Sphere:
    ...
    def _make_bands(self, nlat, nlong):
        ...
        def circle2d(c, r, n):
            ...
            dt = tau/n
            for i in range(n):
                point = ((r*cos(i*dt)+cx), (r*sin(i*dt)+cy))
                points.append(point)
    ...
    def iter_polygons(self):
        ...
        for i in range(len(b0)-1):
            points = [self.southpole, b0[i], b0[i+1]]
            yield Record(points=points, color=self.color)
    ...
    def intersect(self, ray, interval, info):
        ...
        c = (ray.start-self.pos).mag2() - self.radius*self.radius
        discrim = (b*b - 4*a*c)
        if discrim < 0:
            return False
    ...
    def _setinfo(self, ray, t, info):
        ...
        info.normal = (ray.point_at(t)-self.pos).normalized()
...
class Box:
    ...
    def __init__(self, pos, size, color):
        ...
        self.size = size
        self.color = RGB(color)
        xlow, xhigh = pos[0] - (size[0])/2, pos[0] + (size[0])/2
        ylow, yhigh = pos[1] - (size[1])/2, pos[1] + (size[1])/2
        zlow, zhigh = pos[2] - (size[2])/2, pos[2] + (size[2])/2
        self.planes = [(xlow, xhigh), (ylow, yhigh), (zlow, zhigh)]

    def iter_polygons(self):
        xs, ys, zs = self.planes
        yield Record(points=[Point((xs[0],ys[0],zs[0])),
                             Point((xs[0],ys[0],zs[1])),
                             Point((xs[0],ys[1],zs[1])),
                             Point((xs[0],ys[1],zs[0]))], color=self.color) # left
        yield Record(points=[Point((xs[0],ys[1],zs[0])),
                             Point((xs[0],ys[1],zs[1])),
                             Point((xs[1],ys[1],zs[1])),
                             Point((xs[1],ys[1],zs[0]))], color=self.color) # top
        yield Record(points=[Point((xs[1],ys[1],zs[0])),
                             Point((xs[1],ys[1],zs[1])),
                             Point((xs[1],ys[0],zs[1])),
                             Point((xs[1],ys[0],zs[0]))], color=self.color) # right
        yield Record(points=[Point((xs[1],ys[0],zs[0])),
                             Point((xs[0],ys[0],zs[0])),
                             Point((xs[0],ys[0],zs[1])),
                             Point((xs[1],ys[0],zs[1]))], color=self.color) # bottom
        yield Record(points=[Point((xs[1],ys[0],zs[1])),
                             Point((xs[1],ys[1],zs[1])),
                             Point((xs[0],ys[1],zs[1])),
                             Point((xs[0],ys[0],zs[1]))], color=self.color) # back
        yield Record(points=[Point((xs[0],ys[1],zs[0])),
                             Point((xs[1],ys[1],zs[0])),
                             Point((xs[1],ys[0],zs[0])),
                             Point((xs[0],ys[0],zs[0]))], color=self.color) # front
