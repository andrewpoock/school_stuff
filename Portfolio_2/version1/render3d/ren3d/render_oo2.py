...
def render_wireframe(scene, img):
    ...
    d = -camera.distance
    for poly in scene.objects.iter_polygons():
        campoints = [(d*p.x/p.z, d*p.y/p.z) for p in poly.points]
        fb.draw_polygon(campoints, poly.color.quantize(255))
...
class FrameBuffer:
    ...
    def __init__(self, img, window):
        self.img = img
        self.size = img.size
        self.db = defaultdict(lambda:-inf)
    ...
    def set_pixel(self, loc, z, color):
        w, h = self.size
        if 0 <= loc[0] < w and 0 <= loc[1] < h and z > self.db[loc[0], loc[1]]:
            self.db[loc[0], loc[1]] = z
