...
class Box:
    ...
    def compute_generic(self, pt):
        x, y, z = pt
        u = lerp(x, self.planes[0][0], self.planes[0][1], -1, 1)
        v = lerp(y, self.planes[1][0], self.planes[1][1], -1, 1)
        n = lerp(z, self.planes[2][0], self.planes[2][1], -1, 1)
        return (u, v, n)
...
class Square(Box):
    ...
    def __init__(self, color):
        ...
        self.bbox = BoundingBox((-.5,0,-.5),(.5,0,.5))
...
class Sphere:
    ...
    def __init__(self, pos=(0, 0, 0), radius=1,
                 color=(0, 1, 0), nlat=15, nlong=15, texture=None):
        """ create a sphere
        """
        self.pos = Point(pos)
        self.radius = radius
        ...
        self.texture = texture
        self.bbox = BoundingBox((self.pos[0] - self.radius, self.pos[1] - radius,
                                 self.pos[2] - self.radius),(self.pos[0] + self.radius,
                                 self.pos[1] + self.radius, self.pos[2] + self.radius))
    ...
    def compute_generic(self, pt):
        x, y, z = pt
        cx, cy, cz = self.pos
        u = lerp(x, cx-self.radius, cx+self.radius, -1, 1)
        v = lerp(y, cy-self.radius, cy+self.radius, -1, 1)
        n = lerp(z, cz-self.radius, cz+self.radius, -1, 1)
        return (u, v, n)
...
class Group:
    ...
    def add(self, model):
        ...
        self.bbox = self.bbox.combine(model.bbox)
