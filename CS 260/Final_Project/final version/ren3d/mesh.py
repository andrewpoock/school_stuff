...
class Mesh:
    ...
    def __init__(self, fname, color, recenter=False, smooth=False):
        ...
        self.group = make_BVH(list(self._make_tris(smooth, color)))
        self.bbox = self.meshdata.bbox

    def _make_tris(self, smooth, color):
        for i in self.meshdata.face_indexes:
            points = self.meshdata.get_points(i)
            if smooth:
                norms = self.meshdata.get_vertex_normals(i)
            else:
                norms = [self.meshdata.get_face_normal(i) for _ in range(len(points))]
            for tri in _make_triangles(points, norms, color):
                yield tri
...
def _make_triangles(facepoints, facenormals, color):
    ...
    """
    tris = []
    for i in range(1, len(facepoints)-1):
        pts = ([facepoints[0], facepoints[i], facepoints[i+1]])
        norms = ([facenormals[0], facenormals[i], facenormals[i+1]])
        tris.append(Triangle(pts, color, norms))
    return tris
