...
class Keyboard:
    ...
    """A Keyboard "wraps" a synthesizer so that it can easily be used to 
    generate notes from a musical scale.
    """
    ...
    def play_key(self, keynum, duration):
        ...
        post: returns SoundWave of the given keynum and duration
        """
    ...
    def play_keys(self, keys, duration):
        ...
        """
        wave = self.play_key(keys[0], duration)
        for key in keys[1:]:
    ...
    def play_notes(self, notes, duration):
        ...
        """
        wave = self.play_note(notes[0], duration)
        for note in notes[1:]:
            wave += self.play_note(note, duration)
        return wave
    ...
    def play_chord(self, basepitch, intervals, duration):
        ...
        """
        wave = self.play_note(basepitch, duration)
        base = self.keynum(basepitch)
        for n in intervals:
            wave += self.play_key(base + n, duration)
        return wave
